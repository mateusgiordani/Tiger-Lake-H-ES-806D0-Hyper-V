...
- `analysis/scripts/collect_tgl_vmx_msrs_linux.py` — coletor somente leitura
  (já existente; compara todos os LPs online);
- `analysis/scripts/run-in-ubuntu.sh` — novo empacotador que roda o coletor e
...
```bash
cd /caminho/montado/projeto conserto bios/bios-interposer/analysis/scripts
