...
> Experimento histórico concluído: a tabela corrigida chegou ao Windows, mas não
> resolveu a falha do Hyper-V. O resultado atual está em
...
- `firmware/patches/opencore-madt-test/EFI` — árvore pronta;
- `firmware/patches/opencore-madt-test/manifest.json` — hashes e bytes exatos;
- `archive/experiments/legacy-scripts/build_opencore_madt_test.py` — gerador e validador da MADT;
- `archive/experiments/legacy-scripts/deploy_opencore_madt_usb.py` — instalação/restauração reversível;
- `archive/experiments/legacy-scripts/dump_acpi_windows.py` — leitura direta da tabela entregue ao Windows.
