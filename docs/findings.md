...
Evidências em `evidence/msr/linux/vmx-ON` \(`34B6B668...`\) e `vmx-OFF` \(`D084D8F4...`\):
...
.\archive\experiments\legacy-scripts\prepare-hyperv-diagnostic-matrix.ps1 -Apply   # cria + backup em analysis\bcd-backups\hyperv-matrix-*.json
```
