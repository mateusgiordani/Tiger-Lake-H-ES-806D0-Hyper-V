...
As três leituras são idênticas. O dump e o log do FPT foram preservados em `analysis/runtime-dump-2026-08-24/`.
...
O protocolo completo está em `analysis/HIPER_V_MATRIZ_TESTES.md`. O script `analysis/scripts/prepare-hyperv-diagnostic-matrix.ps1` é `dry-run` por padrão e cria, somente com `-Apply` elevado:
...
Foi inventariada a pilha Microsoft relevante. `vmbus.sys`, `storvsc.sys` e `vpci.sys` são `BOOT_START`; `Vid.sys` é `SYSTEM_START`; `intelppm.sys` é PnP/demand start. Todos os arquivos verificados possuem assinatura Microsoft válida. O inventário com versão, hash e grupo está em `analysis/hyperv-root-driver-inventory.json`.
...
O comparador `analysis/scripts/compare_uefi_code_modules.py` agrupa PE32/TE pelo nome do módulo, ignorando índices de caminho que mudam com NVRAM. Resultados gravados em JSON:
...
5. preparar a matriz BCD recuperável descrita em `analysis/HIPER_V_MATRIZ_TESTES.md`, mantendo `{current}` com `hypervisorlaunchtype off` como padrão;
